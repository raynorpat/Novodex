 ...
 * upstream: External/opcode/upstream/Opcode/OPC_AABBTree.cpp
 *
 * Two blocks of NovodeX code, both inside the build path, both absent from
 * stock 1.3. Task 2a vendored this file stock, which put 91% of the phase's
 * third-party bytes behind a silently divergent tree builder: with the added
 * BuildSettings fields left at their defaults the stock file and the image
 * agree, and with either of them set they do not.
 *
 * [1] AABBTree::Build arms a one-shot capture latch on the builder immediately
 *     before it calls _BuildHierarchy.
 *     established at 0x000f10c0 is Build. Its last three instructions before
 *     `call 0x000f0ea0` are `push esi; mov ecx,edi; mov byte ptr [esi+0x38],1`
 *     at 0x000f1187, with esi the builder. Stock has no statement there at all:
 *     the byte is inside the 28 added builder bytes (OPC_TreeBuilders.h [2]).
 *
 * [2] AABBTreeNode::_BuildHierarchy gains a block between ComputeGlobalBox and
 *     Subdivide. Stock is four statements; the image is these:
 *     established at 0x000f0ea0, reading builder in esi and this in edi:
 *       0x000f0ec0  mov edx,[esi+0x10]; cmp edx,-1; je 0x000f0f83
 *                     -- mSettings.mNovodeXExtendAxis, -1 skips to [2b]
 *       0x000f0ecc  mov al,[esi+0x38]; test al,al; je 0x000f0efe
 *                     -- the latch [1] set
 *       0x000f0ed7  lea eax,[esi+0x20]; six dword moves from [ecx+0..0x14]
 *                     -- builder->mNovodeXRootBV = mBV, the ROOT's box only
 *       0x000f0efa  mov byte ptr [esi+0x38],0
 *                     -- one shot: later nodes compare against the root's box
 *       0x000f0efe  fld [esi+0x0c]; fcomp [esi+edx*4+0x20]; fnstsw ax;
 *                   test ah,5; jp
 *                     -- mNovodeXExtendValue against mNovodeXRootBV.mMin[axis].
 *                        `test ah,5` is C0|C2 and `jp` takes even parity, so
 *                        the fallthrough at 0x000f0f3b -- which stores the
 *                        value into the local mBV.mMin[axis] -- is taken on
 *                        strictly-less-than only.
 *       0x000f0f47  fld [esi+0x0c]; fcomp [esi+edx*4+0x2c]; test ah,0x41; jne
 *                     -- and against mNovodeXRootBV.mMax[axis], +0x2c being
 *                        +0x20+0x0c. `jne` on C0|C3 leaves the store at
 *                        0x000f0f52 -- into the local mBV.mMax[axis] -- for
 *                        strictly-greater-than only. Neither branch: the
 *                        write-back is skipped outright.
 *       0x000f0f59  the local mBV written back to [edi] and [edi+0x0c]
 *   [2b]
 *       0x000f0f83  mov eax,[esi+0x14]; test eax,eax; je 0x000f1028
 *                     -- mSettings.mNovodeXInflate, 0.0f skips
 *       0x000f0f8e  fld [esi+0x14] then three fsub against [edi+0..8] and
 *                   three fadd against [edi+0x0c..0x14], to 0x000f1026
 *                     -- mBV.mMin -= margin, mBV.mMax += margin
 *       0x000f1028  push esi; mov ecx,edi; call 0x000f09b0 = stock Subdivide
 *
 *     The gate at 0x000f0f83 is an integer test of the float's bit pattern, so
 *     the image also inflates by -0.0f where `!= 0.0f` below does not. Recorded
...
void AABBTreeNode::_BuildHierarchy(AABBTreeBuilder* builder)
{
	...
	// NOVODEX [2]: extend the node's box to a plane along one axis, then inflate
	// it by a margin. Both are off by default (axis -1, margin 0.0f), which is
	// why vendoring this file stock produced the right tree until something set
	// one of them, and why no test that leaves the defaults alone can tell.
	//
	// Written through AABB's accessors rather than mMin/mMax so that it holds
	// under either IceAABB.h representation. The image's is min/max --
	// AABB::Add at 0x000e2d20 reads [this+0] as the min with no subtraction --
	// but which one this tree gets is a build-configuration question and not
	// this file's to answer.
	if(builder->mSettings.mNovodeXExtendAxis!=-1)
	{
		const udword Axis = (udword) builder->mSettings.mNovodeXExtendAxis;

		// 0x000f0ecc: the root's box, and only the root's, is kept on the builder.
		if(builder->mNovodeXCaptureRootBV)
		{
			builder->mNovodeXRootBV			= mBV;			// 0x000f0ed7
			builder->mNovodeXCaptureRootBV	= false;		// 0x000f0efa
		}

		const float Value = builder->mSettings.mNovodeXExtendValue;
		if(Value < builder->mNovodeXRootBV.GetMin(Axis))	// 0x000f0efe
		{
			Point Min;	mBV.GetMin(Min);
			Point Max;	mBV.GetMax(Max);
			Min[Axis] = Value;								// 0x000f0f3b
			mBV.SetMinMax(Min, Max);
		}
		else if(Value > builder->mNovodeXRootBV.GetMax(Axis))	// 0x000f0f47
		{
			Point Min;	mBV.GetMin(Min);
			Point Max;	mBV.GetMax(Max);
			Max[Axis] = Value;								// 0x000f0f52
			mBV.SetMinMax(Min, Max);
		}
	}

	if(builder->mSettings.mNovodeXInflate!=0.0f)			// 0x000f0f83
	{
		const float Margin = builder->mSettings.mNovodeXInflate;
		Point Min;	mBV.GetMin(Min);
		Point Max;	mBV.GetMax(Max);
		Min.x -= Margin;	Min.y -= Margin;	Min.z -= Margin;	// 0x000f0f8e
...
//	if(builder->mRules&SPLIT_COMPLETE)
	...
	// NOVODEX [1]: arm the one-shot root-box capture. The image sets this in the
	// two instructions before the call below, at 0x000f1187.
	builder->mNovodeXCaptureRootBV = true;
