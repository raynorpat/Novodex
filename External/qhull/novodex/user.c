 ...
 * upstream: External/qhull/upstream/src/user.c
 *
 * [1] qh_errexit's body replaced by a single call on the host object's slot
 *     +0x20 forwarding ONLY the exit code. The stock body is kept below under
 *     `#if 0` so the reader can see what was displaced; the two help printers it
 *     calls, qh_printhelp_degenerate and qh_printhelp_singular, are absent from
 *     the image for exactly this reason.
 *     established at 0x00084800 is seventeen bytes: `mov ecx,[0x10125080]; mov edx,[esp+4];
 *     mov eax,[ecx]; push edx; call dword ptr [eax+0x20]; ret`. `ret` with no
 *     stack adjustment is cdecl, and it forwards only its first argument
 *     although its callers pass three. It is called from 69 distinct qhull
 *     functions.
 *
...
void qh_errexit(int exitcode, facetT *facet, ridgeT *ridge) {
  /* NOVODEX [1]: the whole body is replaced by a call that hands the exit code
     to the host and does not return here. facet and ridge are not forwarded. */
  qhNovodeXErrexit (exitcode);
}

#if 0	/* NOVODEX: stock qh_errexit, kept for the reader, not compiled. */
static void qh_errexit_stock(int exitcode, facetT *facet, ridgeT *ridge) {
...
#endif	/* NOVODEX: end of the displaced stock qh_errexit */
