 ...
 * upstream: External/opcode/upstream/Opcode/OPC_OptimizedTree.h
 *
 * [1] The same three virtuals as on BaseModel, but INSERTED before GetUsedBytes
 *     rather than appended, so AABBOptimizedTree::GetUsedBytes moves from slot 4
 *     to slot 7. The four concrete trees carry stub overrides: the bodies are
 *     census rows Task 2b owns, not reconstructions.
 *     established at the abstract base vtable at .rdata:0x0011bc4c has EIGHT slots where
 *     stock has five -- slot 0 is a real destructor at 0x000f21a0 and slots 1
 *     through 7 are all _purecall (0x000f41dc). The dispatch sites fix the
 *     mapping: 0x000e9410 Model::Refit calls [edx+0x08] (slot 2 = Refit),
...
#define __OPC_OPTIMIZEDTREE_H__
	...
	#define IMPLEMENT_COLLISION_TREE(base_class, node)																\
		...
		/* NOVODEX: the three added slots. NOT RECONSTRUCTED -- Task 2b owns						\
		   the twelve rows behind these four instantiations. */									\
		override(AABBOptimizedTree)	udword			NovodeXSlot4()			{ return 0;					}	\
		override(AABBOptimizedTree)	bool			Save(void*)				{ return false;				}	\
		override(AABBOptimizedTree)	bool			Load(const void*)		{ return false;				}	\
		private:																									\
									node*			mNodes;
	...
	class OPCODE_API AABBOptimizedTree
	{
		...
		// NOVODEX: three virtuals INSERTED at slots 4, 5 and 6, which displaces
		// GetUsedBytes from slot 4 to slot 7. See [1].
		virtual			udword				NovodeXSlot4()												= 0;
		virtual			bool				Save(void* stream)											= 0;
		virtual			bool				Load(const void* stream)									= 0;
